...
Mean Values of a Replicated Sample
...
#gets the averages of a bunch of 4k samples for each dataset
means_of_samples <- function(dataset, platform_name){

  S <- replicate(1000, {
    sample <- sample_n(dataset, 4000, replace = TRUE)
    nrc_test <- console_sentiment_analysis_prep_nrc(sample, platform_name)
  
  
  
  #counts sentiment
  nrc_sentiment_1_day <- nrc_test %>%
    count(platform, sentiment)
  nrc_sentiment_1_day$n
  
  })

  fixed <- as.data.frame(t(S))
  fixed <- fixed %>% rename(Anger = V1, Anticipation = V2, Disgust = V3, Fear = V4, Joy = V5, 
                Negative = V6, Positive = V7, Sadness = V8, Surprise = V9, Trust = V10)
  bootstrapped_emotions <- fixed %>% summarise(Anger = mean(Anger), Anticipation = mean(Anticipation), Disgust = mean(Disgust), 
                                             Fear = mean(Fear), 
                                             Joy = mean(Joy), 
                                             Negative = mean(Negative), Positive = mean(Positive), Sadness = mean(Sadness), 
                                             Surprise = mean(Surprise), 
                                             Trust = mean(Trust))
  bootstrapped_emotions_tidy <- bootstrapped_emotions %>% gather(sentiment) %>% mutate(platform = platform_name)


}


playstation_samples <- means_of_samples(playstation_english, "Playstation")
xbox_samples <- means_of_samples(xbox_x_english, "Xbox_X")
nrc_sentiment <- rbind(playstation_samples, xbox_samples)
...
nrc_sentiment %>% ggplot(aes(x = sentiment, y = value, shape = platform, color = platform)) +
  geom_point(size = 4) +
  coord_polar() +
  scale_y_log10() +
  theme_minimal() +
  geom_text_repel(aes(label = value), size = 3, color = "black", vjust = -1, force = 2) +
